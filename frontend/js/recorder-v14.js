...
// =============================================================================
// VERSION MARKER - V14 (NEW FILENAME) // #claude
// =============================================================================
console.log('%c🔥 RECORDER-V14.JS LOADED 🔥', 'background: #00ff00; color: #000000; font-size: 20px; font-weight: bold; padding: 10px;'); // #claude
console.log('V14: NEW FILENAME - Chunks in MEMORY, inflate BEFORE IndexedDB'); // #claude

// Update visible version badge on screen (no console needed!)
if (document.getElementById('versionBadge')) {
    document.getElementById('versionBadge').textContent = '✅ V14 LOADED';
...
const TEST_MODE = { // #claude
    enabled: false, // #claude - DISABLED: Get basic upload working first, debug inflation later
    inflateFileSize: false, // Artificially increase file size for upload stress testing // #claude
...
async function startRecording() {
    try {
        ...
        mediaRecorder.onstop = async () => { // #claude - made async
            ...
            // #claude v12: TEST MODE - Inflate BEFORE saving to IndexedDB
            if (TEST_MODE.enabled && TEST_MODE.inflateFileSize) {
                ...
                try {
                    ...
                    if (paddingNeeded > 0) {
                        console.log(`  🔧 Adding ${(paddingNeeded / 1024 / 1024).toFixed(2)} MB padding...`);

                        // Create padding blob (random data)
            ...
            // #claude v12: NOW save final blob to IndexedDB (already inflated if TEST_MODE)
            console.log('💾 Saving to IndexedDB...');
            await FacialStorage.saveInflatedBlob(currentVideoId, finalBlob);
            currentVideoSize = finalBlob.size; // #claude v12: Store for handleAccept
            console.log(`✅ Video saved: ${(currentVideoSize / 1024 / 1024).toFixed(2)} MB`);
...
async function handleAccept() { // #claude
    ...
    if (window.backendSessionId && window.uploadManager) { // #claude
        ...
        await window.uploadManager.enqueueVideo(currentVideoId, currentVideo.id, uploadMetadata); // #claude
        console.log('✅ Video enqueued for upload'); // #claude
    } else { // #claude
    ...
    if (hasMoreVideos) { // #claude
        ...
        // Session complete - navigate to completion screen // #claude
...
async function handleRetake() { // #claude - made async
    ...
    // #claude v12: Delete video from IndexedDB (single blob)
    if (currentVideoId) { // #claude
        await FacialStorage.deleteVideo(currentVideoId); // #claude v12
    } // #claude
